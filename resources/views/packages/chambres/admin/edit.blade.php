...
@section('title')
    Chambres - Editer une chambre :: Youpigoo
@endsection
@section('content')
    ...
    <div class="px-4 container-fluid">  
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{url('Admin/chambres/update-chambre/'. $chambre->id_chambre)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-3">
                        <label for="categoriechambre_id">Catégorie de chambre</label>
                        <select name="categoriechambre_id" class="form-control">
                            <option value="categoriechambre_id">... Sélectionner la catégorie ...</option>
                            @foreach ($categoriechambres as $catchambre)
                                <option value="{{ $catchambre->id_categoriechambre}}"
                                    {{ $chambre->categoriechambre_id == $catchambre->id_categoriechambre ? 'selected' : '' }}>{{ $catchambre->libelle_categoriechambre }} </option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select name="pays_id" class="form-control">
                            ...
                            @foreach ($pays as $pays)
                                <option value="{{ $pays->id_pays }}"
                                    {{ $chambre->pays_id == $pays->id_pays ? 'selected' : '' }}>{{ $pays->nom_pays }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select name="ville_id" class="form-control">
                            ...
                            @foreach ($villes as $ville)
                                <option value="{{ $ville->id_ville }}"
                                    {{ $chambre->ville_id == $ville->id_ville ? 'selected' : '' }}>{{ $ville->nom_ville}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="typehebergement_id" class="form-control">
                            <option value="typehebergement_id">... Sélectionner le type ...</option>
                            @foreach ($typehebergements as $typehebergement)
                                <option value="{{ $typehebergement->id_typehebergement}}"
                                    {{ $chambre->typehebergement_id == $typehebergement->id_typehebergement ? 'selected' : '' }}>{{ $typehebergement->nom_typehebergement }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select name="hotel_id" class="form-control">
                            ...
                            @foreach ($hotels as $hotel)
                                <option value="{{ $hotel->id_hotel}}"
                                    {{ $chambre->hotel_id == $hotel->id_hotel ? 'selected' : '' }}>{{ $hotel->nom_hotel }} </option>
                            @endforeach
                        </select>
                    </div>
